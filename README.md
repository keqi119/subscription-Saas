...
- `docs/stage-10d-lease-saas-cps-compliance-system-design.md`: Stage 10D Lease SaaS + Signing + CPS certificate compliance architecture
- `docs/stage-10d-lease-saas-cps-compliance-system-design.zh-CN.md`: Stage 10D 租赁 SaaS + 电子签 + CPS 证书合规中文架构设计
...
- `docs/stage-10x-vehicle-model-read-unification.md`: Stage 10X-R VehicleModel read-path unification layer, making modelDefinitionId the primary runtime read identity while keeping legacy enum as compatibility display.
- `docs/stage-10x-vehicle-model-enum-zero-risk-removal-plan.md`: Stage 10X-S VehicleModel enum zero-risk removal plan covering shadow tracking, dual-read validation, dependency sweep, schema safety gates, rollback, and staged removal without implementation changes.
- `docs/stage-10x-vehicle-model-runtime-evidence.md`: Stage 10X-T VehicleModel runtime evidence system with shadow telemetry, business decision tracing, external dependency scan, readiness scoring, and local JSON report generation.
- `docs/stage-10x-vehicle-model-schema-contract-deprecation-plan.md`: Stage 10X-U VehicleModel schema contract deprecation plan covering API contract deprecation, CSV/report migration, schema removal gates, rollback, and phased warning/removal windows.
- `docs/stage-10x-vehicle-model-contract-deprecation-governance.md`: Stage 10X-U-A external VehicleModel contract deprecation governance covering API versioning, consumer registry, CSV/report schema versioning, warning mode, removal gates, and rollback strategy.
- `docs/stage-10x-vehicle-model-external-migration-enforcement.md`: Stage 10X-U-B VehicleModel external contract migration enforcement with registry coverage checks, warning telemetry, readiness scoring, and release gate integration.
...
- `docs/stage-10x-product-price-rule-constraint-migration.md`: Stage 10X-V ProductPriceRule additive uniqueness migration to `modelDefinitionId`, retaining legacy `vehicleModel` uniqueness for compatibility while adding readiness and pricing integrity gates.
- `docs/stage-10x-product-price-rule-constraint-decommission.md`: Stage 10X-W ProductPriceRule legacy `productVersionId + vehicleModel` unique constraint decommission, with migration safety, rollback, and validation gates.
- `docs/stage-10x-vehicle-model-final-zero-risk-decommission.md`: Stage 10X final VehicleModel enum zero-risk schema decommission plan, including dependency verification, hard gates, migration sequence, rollback, cutover, and current NOT READY evidence.
