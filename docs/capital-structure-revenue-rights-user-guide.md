...
9. 收益权 assignment 表单按 targetType 只显示一个目标对象字段：ORDER 显示订单，RECEIVABLE_BILL 显示账单，VEHICLE 显示车辆。
10. 收益权 assignment 中，PLEDGE / TRANSFER / SPV_POOL 需要选择融资工具；REVENUE_SHARE / OTHER 不强制融资工具。
11. 分润规则按 ruleType 动态展示字段：收益分成显示分成比例，固定租金显示固定月金额，混合规则同时显示分成比例和固定金额。
12. 车主分成比例与平台分成比例合计不等于 100% 时，页面会提示复核，但不强制阻止提交。
