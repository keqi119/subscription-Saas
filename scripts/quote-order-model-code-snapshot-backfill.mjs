...
const outputDir = resolve(repoRoot, ".tmp/quote-order-model-code-snapshot-backfill");
...
if (!databaseUrl) {
  throw new Error("DATABASE_URL is required for Quote / Order model code snapshot backfill.");
}
...
async function main() {
  ...
  if (mode === "apply") {
    if (hasBlockingIssues(plans)) {
      applyBlocked = true;
      applyError =
        "Quote / Order model code snapshot backfill apply blocked because unresolved or conflict records exist.";
    } else {
...
async function buildDryRunPlan() {
  ...
  const orderPlan = buildOrderModelCodeSnapshotPlan({ orders, quotePlan });
...
async function applyQuotePlan(tx, plan) {
  ...
  for (const update of plan.updates) {
    const result = await tx.subscriptionQuote.updateMany({
      data: {
        legacyVehicleModelCodeSnapshot: update.legacyVehicleModelCodeSnapshot
      },
      where: {
        id: update.id,
...
function buildReport({ applyBlocked, applyError, environment, mode, plans }) {
  ...
  return {
    ...
    applyError,
    auditNote:
      "legacyVehicleModelCodeSnapshot is an additive historical explanation field. Backfilled values preserve existing string compatibility snapshots and do not alter original quote/order facts.",
    environment,
...
function renderMarkdownReport(report) {
  const lines = [
    "# Quote / Order Model Code Snapshot Backfill Report",
    "",
  ...
  lines.push(
    ...
    "It does not replace the original string vehicleModel or legacyVehicleModelSnapshot.",
    "Backfilled values preserve existing string compatibility snapshots and do not alter original quote/order facts.",
    "",
    "## Out of Scope",
    "",
...
function environmentSummary() {
  ...
  return {
    allowProductionBackfill:
      process.env.ALLOW_PRODUCTION_QUOTE_ORDER_MODEL_CODE_SNAPSHOT_BACKFILL === "1",
    appEnv,
    isProduction,
    nodeEnv
  };
