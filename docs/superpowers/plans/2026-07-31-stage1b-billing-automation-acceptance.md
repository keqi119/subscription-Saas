...
## 8. 2026-08-03 已交付订单租约与账单计划恢复

本次修复覆盖已确认交付、订单状态为 `ACTIVE`，但历史交付路径未创建 `Lease` 与 `BillingSchedule` 的订单。修复不新增 migration，不改写既有合法账单、支付、里程或交付事实。

受影响的 staging 验收订单：

- `ORD20260726073922TFHF`
- `ORD20260731173351SMF2`
- `ORD20260802071556GFEY`

部署后按以下顺序恢复：

1. 保持 `BILLING_AUTOMATION_WORKER_ENABLED=false`，完成 API/Web 镜像切换和健康检查。
2. 进入“月租账单自动化”，执行“协调预览”。上述订单应进入符合条件列表；缺失租约显示“预计恢复租约”，缺失计划显示“预计创建”。
3. 核对下一账期以订单 `actualDeliveryAt` 的上海业务日期为锚点，生成日为账期开始日前 D-3；`ORD20260802071556GFEY` 应显示账期 `2026-09-02` 至 `2026-10-01`、生成日 `2026-08-30`。
4. 执行“账单计划协调”。系统在单个数据库事务内恢复 `ACTIVE` Lease 并创建唯一 BillingSchedule，同时记录系统审计。
5. 再次执行协调预览。三张订单均应显示已有计划、租约正常，预计新增和预计恢复数量均为 0。
6. 启用 Worker 后，确认 D-3 到达时只生成一张对应账期账单；重复扫描不产生重复 Lease、计划、任务或账单。
7. 人工“应急兜底”试算与自动计划使用同一 D-3 边界：D-4 显示未到生成日期，D-3 显示可生成。
