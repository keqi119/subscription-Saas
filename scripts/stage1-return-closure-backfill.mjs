...
export async function loadSnapshot(client) {
  ...
  return { audits, bills, clauses, closures, contracts, damages, deliveries, dispositions, files, links, settlements };
}
...
export async function applyClassification(client, classification) {
  ...
  let publicationConstraintValidated = false;
  if (classification.publicationValidationReady) {
    ...
    publicationConstraintValidated = true;
  }
  return { batchSize: 100, publicationConstraintValidated, ...counts };
}
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  main()
    ...
    .catch((error) => {
      process.stderr.write(`${JSON.stringify({ error: error instanceof Error ? error.message : "BACKFILL_FAILED" })}\n`);
      process.exitCode = 1;
    })
