...
| 订单无争议正常完成                                 | Closure `COMPLETED`、已结束 Lease/Period、车辆取回及库存释放事实的完整组合                                               | `Order.COMPLETED` 和 `Vehicle.AVAILABLE/RETURNED` 是结果投影                                                   |
| 订单提前终止的共同不可伪造门槛                     | 至少包括对应 Closure 终态、已结束 Lease/Period、车辆取回及库存释放事实；具体终止决定、费用、争议和验收语义留待 S5 规格   | `Order.TERMINATED` 不能单独证明提前终止完整闭环；S0 不批准其详细终态矩阵                                       |
...
- `COMPLETED` 只能由无争议正常结束的 Closure `COMPLETED`、Lease/Period 结束及车辆释放的完整组合派生；
- `TERMINATED` 在 S0 只受共同的不可伪造最低门槛约束。提前终止的详细决定、费用、争议、投影条件和验收矩阵归 S5；S2 不修改或验收该语义。
...
| 订单 `COMPLETED`（无争议正常结束）        | Closure `COMPLETED` + Lease/Period 结束 + 车辆释放组合                            | `SubscriptionReturnGovernanceService.completeOperations` 的正常结束路径                                                                                                     | Order 列表/工作台、Billing/报表按 OrderStatus 的读取                                                                                                                                                 | 当前终态写入与 Closure 同事务，但大量读取者只看 OrderStatus；需要固定正常完成的不变量                                    | S2 转换 6：仅无争议正常结束                          | 新增正常完成终态契约；任一 Closure、Lease、Period、Return、Restriction 条件缺失均不得报告 `COMPLETED`                                                   |
| 订单 `TERMINATED`（提前终止等非正常结束） | 共同最低门槛为相应 Closure 终态 + Lease/Period 结束 + 车辆释放组合；详细语义待 S5 | `SubscriptionReturnGovernanceService.completeOperations` 及其他提前终止/Closure 终态路径                                                                                    | Order 列表/工作台、Billing/Change/报表按 OrderStatus 的读取                                                                                                                                          | 当前读取者仍可能只看 OrderStatus；S0 只刻画不可伪造原则，不定义提前终止的决定、费用、争议或验收矩阵                      | S5 独立能力规格；S2 禁止修改或验收                   | S0 仅保留现有测试和“仅投影不得证明闭环”的特征化；详细终态矩阵、修复和验收由 S5 规格定义                                                                 |
...
| `flag.journey-stage1`                        | 阶段性功能开关   | `SUBSCRIPTION_JOURNEY_ENABLED`、`SUBSCRIPTION_JOURNEY_WORKER_ENABLED`、两个 `SUBSCRIPTION_JOURNEY_ALLOWLIST_*`                                                                              | Stage1 发布负责人                              | `6415d4df`                                    | Journey enrollment、Worker 和受控验收范围                                                                  | 两个布尔开关均仅精确 `true` 开启，缺失/其他值关闭；allowlist 缺失/为空时在 Journey 开启后不限制对象；S3 仅对冻结 RC 和批准的 Manifest 引用启用 | Stage1 签字且 S4 证明无需 allowlist 双路径后，移除 allowlist 与临时分支；Journey 长期启停策略另行评审      | 2026-09-15 | 每次 RC 的非敏感配置指纹随执行证明保留 180 日                 | 阶段保留                                         |
...
| `flag.billing-maintenance-evidence`          | 阶段性功能开关   | `BILLING_MAINTENANCE_EVIDENCE_ENABLED` 及 run/release/image/database identity 元数据                                                                                                        | 计费发布负责人                                 | `96a712e7`、`d31d69b3`                        | Billing maintenance service 写入两次受控维护证据；evidence CLI 读取                                        | 仅精确 `true` 开启，缺失/其他值关闭；只允许与冻结 RC、目标数据库和批准 run id 匹配的短时执行窗口开启                                           | Stage1 签字后立即关闭并清空运行元数据；S4 决定历史只读保留或前向泛化                                       | 2026-09-15 | 执行证明及只读事实保留 180 日，计费发布/审计角色可访问        | 阶段保留，签字后停写                             |
| `flag.stage2-handover`                       | 阶段性功能开关   | `STAGE2_HANDOVER_WORKFLOW_ENABLED`、`STAGE2_HANDOVER_WORKER_ENABLED`                                                                                                                        | 车辆交付发布负责人                             | `c9115672`                                    | Handover、Stage2 ESign、Workflow service 和 Stage2 worker                                                  | 两个开关均仅精确 `true` 开启，缺失/其他值关闭；冻结 RC 仅在交付前置、回滚点和浏览器冒烟证据齐备后共同启用                                      | Stage1 交付签字且 S4 证明旧交付写入者清零、单一路径稳定一个发布周期后折叠                                  | 2026-09-15 | 配置指纹、启停批准和交付冒烟证据保留 180 日                   | 阶段保留，默认关闭                               |
| `flag.contract-artifact-and-esign-rollout`   | 阶段性功能开关   | `CONTRACT_PDF_ARTIFACT_GENERATION_ENABLED`、`ESIGN_STAGE1_MULTI_SLOT_ENABLED`、`ESIGN_ENTERPRISE_AUTO_SEAL_ENABLED`                                                                         | 合同与电子签发布负责人                         | `44e81e18`、`5bc44541`、`35976794`            | Order 合同 PDF artifact、Stage1 多签位和企业自动签章分支                                                   | PDF artifact 仅精确 `true` 开启；另两个开关接受 `1/true/yes/on`，缺失/其他值关闭；仅按冻结 RC 的签署方案批准                                   | Stage1 签字且 S4 证明模板、签位、盖章及归档仅剩单一路径并稳定一个发布周期后折叠                            | 2026-09-15 | 配置指纹、签署冒烟和归档证据保留 180 日                       | 阶段保留，默认关闭                               |
| `flag.acceptance-workers`                    | 阶段性功能开关   | `BILLING_AUTOMATION_WORKER_ENABLED`、`FIELD_VIDEO_UPLOAD_WORKER_ENABLED`、`MILEAGE_REVIEW_WORKER_ENABLED`                                                                                   | 平台运行负责人                                 | 当前 main 基线 `2b96822d`                     | 验收库切换、迁移、回滚和 RC 演练时控制三个辅助 Worker；Runbook 仅引用但不拥有 Journey、Change、Stage2 开关 | 三个开关均仅精确 `true` 开启，缺失/其他值关闭；只按 Runbook 的停写/启用序列和执行批准变更                                                      | S1 将其纳入 Runner/Manifest 后，决定哪些保留为永久运营配置、哪些折叠；在此之前不得删除                     | 2026-09-15 | 每次环境变更的配置指纹和批准记录保留 180 日                   | 阶段保留，待 S1 分类                             |
...
下列配置是 Stage1 主路径或禁止域的长期环境契约，不属于六类临时资产，也不创建第二个资产 owner。表中的 owner 负责环境契约和发布证明；相关业务事实仍服从前述权威矩阵。未来若改变主动支付、电子签供应商或通知渠道，必须由新的 ADR 明确替代本契约，不能通过临时 Runbook 覆盖。

| contractId                         | 配置边界                                                                                    | 当前运行时默认/解析语义                                                                                                                                                | Stage1 冻结 RC 要求                                                                                                                      | owner / 证明                                                                       |
| ---------------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `env.stage1-active-payment`        | `PAYMENT_PROVIDER`、`PAYMENT_MOCK_ENABLED`、`PAYMENT_DEFAULT_CHANNEL`、`WECHAT_PAY_ENABLED` | production 缺少 provider 会启动失败；非 production provider 默认 `mock`；两个布尔值仅精确 `true` 开启；production 默认 channel 为 `MOCK`，因而未显式配置会 fail closed | `PAYMENT_PROVIDER=wechat_pay`、`PAYMENT_MOCK_ENABLED=false`、`PAYMENT_DEFAULT_CHANNEL=WECHAT_JSAPI`、`WECHAT_PAY_ENABLED=true`           | 支付发布负责人；构建/执行证明保存非敏感配置指纹、启动结果和微信 JSAPI 冒烟         |
| `env.stage1-auto-debit-prohibited` | `AUTO_DEBIT_ENABLED`、`PAYMENT_MANDATE_PROVIDER`、`PAYMENT_MANDATE_MOCK_ENABLED`            | 两个布尔值仅精确 `true` 开启；provider 缺失默认 `disabled`；任一启用 auto-debit、非 `disabled` provider 或启用 mandate mock 都使 Stage1 配置校验失败                   | `AUTO_DEBIT_ENABLED=false`、`PAYMENT_MANDATE_PROVIDER=disabled`、`PAYMENT_MANDATE_MOCK_ENABLED=false`；主动支付基线不得存在委托代扣依赖  | 支付发布负责人；执行证明保存禁止域预检、启动结果和配置指纹                         |
| `env.stage1-esign-provider`        | `ESIGN_PROVIDER`、`FADADA_ENABLED`、`FADADA_ENV`、Fadada production URL 与凭证引用          | provider 缺失默认 `mock`；`FADADA_ENABLED` 接受 `1/true/yes/on`，缺失/其他值关闭；`FADADA_ENV` 缺失/非 `production` 默认 `sandbox`；必需凭证缺失时配置加载失败         | `ESIGN_PROVIDER=fadada`、`FADADA_ENABLED=true`、`FADADA_ENV=production`，生产 URL、回调/返回地址和受控 secret 引用齐备；证据不得记录秘密 | 合同与电子签发布负责人；执行证明保存脱敏配置指纹、签署/归档冒烟和 artifact digest  |
| `env.stage1-notification-provider` | `NOTIFICATION_PROVIDER`、`NOTIFICATION_WECHAT_ENABLED` 及微信公众号凭证引用                 | provider 缺失或不匹配微信别名时使用 mock；微信启用值仅精确 `true` 开启，缺失/其他值关闭                                                                                | `NOTIFICATION_PROVIDER=wechat_official_account`、`NOTIFICATION_WECHAT_ENABLED=true`，受控凭证引用齐备；生产验收不得以 mock 作为成功证据  | 通知发布负责人；执行证明保存脱敏配置指纹、真实渠道冒烟和站内通知关联，不保存 token |

`flag.contract-artifact-and-esign-rollout` 管理同一供应商契约内的 artifact、多签位和自动盖章分支；`env.stage1-esign-provider` 管理供应商、环境及凭证准入。两者回答不同命题，不得互相代替。`flag.acceptance-workers` 的 Runbook 可以编排 `flag.journey-stage1`、`flag.contract-change` 和 `flag.stage2-handover` 的启停顺序，但不得修改其 owner 或退出条件。
