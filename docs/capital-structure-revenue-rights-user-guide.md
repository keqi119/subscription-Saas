...
4. 融资工具车辆分摊是当前资本结构事实，资本事件是生命周期时间轴事实。融资分摊可以在车辆详情中补录为资本事件。
5. 融资工具字段均应通过下拉选择，不建议手填 UUID 或 `FI2026...` 业务编号。
6. 添加车辆分摊时，只填写车辆、分摊本金、生效日期和备注。融资工具占用比例由系统按“分摊本金 / 融资工具本金”自动计算，单车融资覆盖率由系统按“分摊本金 / 车辆采购价”展示。
7. 收益权 assignment 表单按 targetType 只显示一个目标对象字段：ORDER 显示订单，RECEIVABLE_BILL 显示账单，VEHICLE 显示车辆。
8. 收益权 assignment 中，PLEDGE / TRANSFER / SPV_POOL 需要选择融资工具；REVENUE_SHARE / OTHER 不强制融资工具。
9. 分润规则按 ruleType 动态展示字段：收益分成显示分成比例，固定租金显示固定月金额，混合规则同时显示分成比例和固定金额。
10. 车主分成比例与平台分成比例合计不等于 100% 时，页面会提示复核，但不强制阻止提交。
